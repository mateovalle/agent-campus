...
export function readLayoutFromFile(): Record<string, unknown> | null {
  ...
  try {
    ...
    console.error('[Agent Campus] Failed to read layout file:', err);
    return null;
  }
}
...
export function writeLayoutToFile(layout: Record<string, unknown>): void {
  ...
  try {
    ...
    console.error('[Agent Campus] Failed to write layout file:', err);
  }
}
...
export function watchLayoutFile(
  ...
  function checkForChange(): void {
    ...
    try {
      ...
      console.log('[Agent Campus] External layout change detected');
      onExternalChange(layout);
    } catch (err) {
      console.error('[Agent Campus] Error checking layout file:', err);
    }
  }
