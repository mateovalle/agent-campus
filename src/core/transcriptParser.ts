    ...
    if (hasToolUse) {
      ...
      for (const block of blocks) {
        if (block.type === 'tool_use' && block.id) {
          ...
          console.log(`[Agent Campus] Agent ${agentId} tool start: ${block.id} ${status}`);
  ...
  if (msgType === 'assistant') {
    ...
    for (const block of content) {
      if (block.type === 'tool_use' && block.id) {
        ...
        const status = formatToolStatus(toolName, block.input || {});
        console.log(
          `[Agent Campus] Agent ${agentId} subagent tool start: ${block.id} ${status} (parent: ${parentToolId})`,
        );
    ...
    for (const block of content) {
      if (block.type === 'tool_result' && block.tool_use_id) {
        console.log(
          `[Agent Campus] Agent ${agentId} subagent tool done: ${block.tool_use_id} (parent: ${parentToolId})`,
        );
