...
**Rotation groups**: `buildDynamicCatalog()` builds `rotationGroups` Map from assets sharing a `groupId`. Flexible: supports 2+ orientations (e.g., front/back only). Editor palette shows 1 item per group (front orientation preferred). `getRotatedType()` cycles through available orientations. First generated variants shipped (sprites4.ts): chair_office/chair_gamer ×4 orientations, couch + monitor_dual back, desk_l mirrored — front variants keep their original ids for old-layout compat, and `layoutToSeats()` treats `orientation: 'front'` as non-explicit so the adjacent-desk facing heuristic still wins for pre-rotation layouts.
